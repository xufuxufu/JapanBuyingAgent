...
from app.models import Product, ProductOperationLog, Receipt, ReceiptBatch, ReceiptItem
...
def test_products_page_has_mobile_cards_sticky_desktop_action_and_local_font_mode(client):
    http, db, _ = client
    product = Product(jan="00123457", name_cn="手机卡片商品", purchase_price=880, internal_sku="NJ-TEST-CARD")
    db.add(product)
    db.commit()
